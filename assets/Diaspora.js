...
var Home = location.href, Pages = Number.MAX_VALUE, xhr, xhrUrl = "";
...
var Diaspora = {
    ...
    HS: function (tag, flag) {
        var id = tag.data("id") || 0, url = tag.attr("href"), title = tag.attr("title") || tag.text();
...
$(function () {
    ...
    $(window).on("scroll", function () {
        if ($(".scrollbar").length && !Diaspora.P()) {
            var st = $(window).scrollTop(), ct = $(".content").height();
            if (st > ct) {
                st = ct;
            }
        ...
        switch (true) {
            ...
            case tag.indexOf("comment") != -1:
                ...
                (function () {
                    ...
                    s.src = "//" + $("#disqus_username").text() + ".disqus.com/embed.js";
                    // Replace `example` with your Disqus username
            ...
            case tag.indexOf("icon-like") != -1:
                var t = $(e.target).parent(), classes = t.attr("class");
                ...
                $.ajax({
                    ...
                    data: "likepost=" + id[1],
                    success: function () {
                        var text = $("#like-" + id[1]).html(), patt = /(\d)+/, num = patt.exec(text);
                        num[0]++;
